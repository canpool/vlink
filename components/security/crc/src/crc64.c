...
static uint64_t crc64_calc(uint64_t origin, const uint8_t *data, size_t len)
{
    uint64_t        crc = origin;
    ...
    for (i = 0; i < len; i++) {
        crc = (crc << 8) ^ crc64_table[((crc >> 56) ^ (uint64_t)*ptr++) & 0x00000000000000FFULL];
    }
...
{
    return crc64_calc(CRC_START_64_ECMA, data, len);
}

uint64_t crc64_we(const uint8_t *data, size_t len)
{
    uint64_t crc = crc64_calc(CRC_START_64_WE, data, len);

    return crc ^ 0xFFFFFFFFFFFFFFFFULL;
}

void crc64_gen_tab(uint64_t table[256])
{
    uint64_t i;
    uint8_t  j;
    uint64_t tmp;
    ...
    for (i = 0; i < 256; i++) {
        ...
        for (j = 0; j < 8; j++) {
            if ((crc ^ tmp) & 0x8000000000000000ULL) {
