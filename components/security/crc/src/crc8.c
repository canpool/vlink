...
static uint8_t crc8_calc(uint8_t origin, const uint8_t *data, size_t len)
{
    uint8_t         crc = origin;
...
{
    return crc8_calc(CRC_START_8, data, len);
}
...
void crc8_gen_tab(uint8_t table[256])
{
    uint16_t i, j;
    uint8_t  crc;
    uint8_t  ch;
