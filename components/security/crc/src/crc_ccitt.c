...
static uint16_t crc_ccitt_calc(uint16_t origin, const uint8_t *data, size_t len)
{
    uint16_t        crc = origin;
    ...
    for (i = 0; i < len; i++) {
        crc = (crc << 8) ^ crc_ccitt_table[((crc >> 8) ^ (uint16_t)*ptr++) & 0x00FF];
    }
...
{
    return crc_ccitt_calc(CRC_START_CCITT, data, len);
}

uint16_t crc_xmodem(const uint8_t *data, size_t len)
{
    return crc_ccitt_calc(CRC_START_XMODEM, data, len);
}

uint16_t crc_ccitt_update(uint16_t crc, uint8_t ch)
{
	return (crc << 8) ^ crc_ccitt_table[((crc >> 8) ^ (uint16_t)ch) & 0x00FF];
}

void crc_ccitt_gen_tab(uint16_t table[256])
{
    uint16_t i, j;
    uint16_t crc;
    uint16_t ch;
