...
usage_str = '''
prj [-h] [-l] [-w <proj_dir> <bsp_name> [-f]] [-m] [-s <CONFIG_AA=x> [<CONFIG_BB=x> ...]]
...
def main():
    ...
    parser.add_argument('-l', '--listbsps',   action = 'store_true')
    ...
    if args.listbsps:
        print("%s" % list(libconf.get_bsps().keys()))
        quit()
    ...
    with open("build.mk", "w", encoding = "utf-8") as f:
        ...
        f.write("cflags-l-y := " + " ".join(list(set(project["cflags-l"]))) + "\n")
        f.write("cflags-g-y := " + " ".join(list(set(project["cflags-g"]))) + "\n")
