...
char *sperm(int mode)
{
    ...
    if ((mode & S_IRUSR) == S_IRUSR) { s[1] = 'r'; }
    if ((mode & S_IWUSR) == S_IWUSR) { s[2] = 'w'; }
    if ((mode & S_IXUSR) == S_IXUSR) { s[3] = 'x'; }

    if ((mode & S_IRGRP) == S_IRGRP) { s[4] = 'r'; }
    if ((mode & S_IWGRP) == S_IWGRP) { s[5] = 'w'; }
    if ((mode & S_IXGRP) == S_IXGRP) { s[6] = 'x'; }

    if ((mode & S_IROTH) == S_IROTH) { s[7] = 'r'; }
    if ((mode & S_IWOTH) == S_IWOTH) { s[8] = 'w'; }
    if ((mode & S_IXOTH) == S_IXOTH) { s[9] = 'x'; }
...
void list_stat(const char *d_name)
{
    char full_path[128] = {0};  // with mount path
    char sub_path[128] = {0};   // remove mount path
    struct stat st;
    ...
    while (1) {
        ...
        // add prefix
#ifdef CONFIG_SPIFFS
        snprintf(full_path, sizeof(full_path), FS_PATH("/%s"), entry->d_name);
#else
...
#else
        snprintf(sub_path, sizeof(sub_path), "%s/%s", d_name + strlen(FS_PATH("/")), entry->d_name);
#endif
